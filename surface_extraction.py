...
class SurfaceExtraction:
    ...
    def z_profile_fft(self):
        ...
        norm_mat = np.tile(np.max(tempt, axis=1)-np.min(tempt, axis=1),
                           [self.w*self.h, 1]).T
        ...
        for i in range(k):
            for j in range(k):
                if i == window_size and j == window_size:
                    ...
                    base_var += (img_p2[i:sz1+i, j:sz2+j] - img_mean) * \
                     (img_p2[i:sz1+i, j:sz2+j] - img_mean)
        return base_var
    ...
    def parameter_setting(self):
        ...
        if self.cluster_method == 'Projection':
            ...
            raise ValueError("cluster_method " +
                             self.cluster_method + " doesn't exist!")
        ...
        ncf, bcf = np.histogram(valm[ind_sig],
                                np.linspace(valm_min, valm_max, 101))
        ncf = ncf/np.sum(ncf)
        ind_uk = np.where(np.logical_and(edge_flag2 > 0, edge_flag2 < 1))
        # ind_uk = np.where(edge_flag2 == 0.5)
        ncb, bcb = np.histogram(valm[ind_uk],
                                np.linspace(valm_min, valm_max, 101))
        ncb = ncb/np.sum(ncb)
        ...
        mold = base/((2 * self.window_size + 1) *
    ...
    def surface_smooth(self, lambda1, edge_flag2, ind_max):
        ...
        #  Calculate step size
        KE = np.max(ind_max[edge_flag2 > 0]) -\
            np.min(ind_max[edge_flag2 > 0]) + 1
        step = KE/100
        ...
        for rn in range(len(ratio)):
            ...
            sratio = np.array([np.round(ratio[rn]*self.h),
                              np.round(ratio[rn]*self.w)])
            sratio = sratio.astype('int')
            srn = ratio[rn] * ratio[rn]

            ind_maxk = cv2.resize(ind_maxk, (sratio[0], sratio[1]),
                                  interpolation=cv2.INTER_NEAREST)
            ind_maxk2 = cv2.resize(ind_max, (sratio[0], sratio[1]),
                                   interpolation=cv2.INTER_NEAREST)
            edge_flag3_2 = cv2.resize(edge_flag3, (sratio[0], sratio[1]),
                                      interpolation=cv2.INTER_NEAREST)
            edge1_2 = cv2.resize(edge1_map, (sratio[0], sratio[1]),
                                 interpolation=cv2.INTER_NEAREST)
            edge5_2 = cv2.resize(edge5_map, (sratio[0], sratio[1]),
                                 interpolation=cv2.INTER_NEAREST)
            edge0_2 = cv2.resize(edge0_map, (sratio[0], sratio[1]),
                                 interpolation=cv2.INTER_NEAREST)
            shiftc2 = cv2.resize(shiftc, (sratio[0], sratio[1]),
                                 interpolation=cv2.INTER_NEAREST)
            ...
            while np.abs(cost[itern]) > 0.001*KE*lim*self.THRES:
                ...
                cost_val = np.sum(np.abs(dt[ind_edge1_2]))/(srn*npxl1) +\
                    np.sum(np.abs(dt[ind_edge5_2]))/(srn*npxl5) +\
                    np.sum(np.abs(dt[ind_edge0_2]))/(srn*npxl0)
...
class RapidSurfaceExtraction(SurfaceExtraction):
    ...
    def rapid_parameter_setting(self):
        """
        Rapid parameter initialization for gradient descent-based
        surface smoothing - parallel computing.
        ...
        ncf, bcf = cupy.histogram(valm[ind_sig],
                                  cupy.linspace(valm_min, valm_max, 101))
        ncf = ncf/cupy.sum(ncf)
        ind_uk = cupy.where(cupy.logical_and(edge_flag2 > 0, edge_flag2 < 1))
        # ind_uk = np.where(edge_flag2 == 0.5)
        ncb, bcb = cupy.histogram(valm[ind_uk],
                                  cupy.linspace(valm_min, valm_max, 101))
        ...
        #  Calculate neighboring mean and variance of initial surface
        class3 = self.rapid_local_sum_class(edge_flag2, self.window_size,
                                            sum_pos=1)
    ...
    def surface_smooth(self, lambda1, edge_flag2, ind_max):
        ...
        #  Calculate step size
        KE = np.max(ind_max[edge_flag2 > 0]) - np.min(ind_max[edge_flag2 > 0])\
            + 1
        step = KE/100
        ...
        for rn in range(len(ratio)):
            ...
            if rn == 0:
                ...
                zoom_scale = [ind_maxk2.shape[0]/ind_maxk.shape[0],
                              ind_maxk2.shape[1]/ind_maxk.shape[1]]
            ...
            while np.abs(cost[itern]) > 0.0001*KE*lim*self.THRES:
                ...
                cost_val = cupy.sum(cupy.abs(dt[ind_edge1_2]))/(srn*npxl1) +\
                    cupy.sum(cupy.abs(dt[ind_edge5_2]))/(srn*npxl5) +\
                    cupy.sum(cupy.abs(dt[ind_edge0_2]))/(srn*npxl0)
